...
public class QuotaDataOperation extends Operation{
	...
	{
		try {
			super.createTable("create table QuotaData_"+siid+"("
	...
	public void updateTable(String type,String siid)
	{
		...
		hisList=historyDataOperation.selectCondition(siid,"where id<="+hisid);
		quoList=quotaDataOperation.selectCondition(siid,"where id<="+quoid);
...
//		if(newquoList.size()>10)
//		{
//			int counter=0;
//			while(counter<newquoList.size())
//			{
//				new QuotaDataOperation().insert(siid,attributes,newquoList.subList(counter, Math.min(counter+10,newquoList.size())));
//				counter+=10;
//			}
//		}
		new QuotaDataOperation().insert(siid,attributes,newquoList);
		
		if(type.equals("s"))
		{
			stockInfOperation.modify("quoid="+hisid+" where sid='"+siid+"'");
		}
		else if(type.equals("i"))
		{
			indiceInfOperation.modify("quoid="+hisid+" where iid='"+siid+"'");
		}
	}

	public void insert(String siid,List<String> attributes,List<QuotaData> quotaDataList)
	{
		String sql="insert into QuotaData_"+siid+"(";
...
//		System.out.println("select * from HistoryData_"+siid+" "+condition);
		...
		try {
			list = QueryTool.resultSetToList(super.select(conn,"select * from QuotaData_"+siid+" "+condition));
			if(list.size()==0)
				return null;
...
//		if(type.equals("s"))
//		{
//			date=stockInfOperation.selectAllSiid(siid).getDate();
//		}
//		else if(type.equals("i"))
//		{
//			date=indiceInfOperation.selectAllSiid(siid).getDate();
//		}
//		if(date==null)
//		{
//			return null;
