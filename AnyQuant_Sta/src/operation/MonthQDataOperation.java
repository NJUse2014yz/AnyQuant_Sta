...
public class MonthQDataOperation extends Operation{
	...
	public static MonthHDataOperation monthHDataOperation=new MonthHDataOperation();
	public static MonthQDataOperation monthQDataOperation=new MonthQDataOperation();
	...
	{
		try {
			super.createTable("create table MonthQData_"+siid+"("
	...
	public void updateTable(String type,String siid)
	{
		...
		hisList=monthHDataOperation.selectCondition(siid,"where monthid<="+hisid);
		quoList=monthQDataOperation.selectCondition(siid,"where monthid<="+quoid);
		...
		if(type.equals("s"))
		{
			stockInfOperation.modify("monthquoid="+hisid+" where sid='"+siid+"'");
		}
		else if(type.equals("i"))
		{
			indiceInfOperation.modify("monthquoid="+hisid+" where iid='"+siid+"'");
		}
	}
	
	public void insert(String siid,List<String> attributes,List<QuotaData> quotaDataList)
	{
		String sql="insert into MonthQData_"+siid+"(";
...
//		System.out.println("select * from HistoryData_"+siid+" "+condition);
		...
		try {
			list = QueryTool.resultSetToList(super.select(conn,"select * from MonthQData_"+siid+" "+condition));
			conn.close();
			if(list.size()==0)
				return null;
	...
	//			System.out.println(hm.get("date").getClass());
				quotaDataList.add(new QuotaData(
						(int)hm.get("monthid"),(Date)hm.get("date"),
